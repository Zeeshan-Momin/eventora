    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          trend={stats.publishedEvents > 0 ? "Active" : undefined}
          trendUp={true}
          delay={200}
        />
        <StatCard
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="grid sm:grid-cols-1 gap-3">
            {insights.map((insight, i) => (
              <div
                key={i}
                className={cn("rounded-2xl p-5 border flex gap-4 items-start animate-fade-up opacity-0 transition-all hover:scale-[1.01]")}
                style={{
                  background: `linear-gradient(135deg, ${insight.color.replace("from-", "").replace("to-", ", ")})`,
              ...
              >
                ...
                <Link href={insight.href}>
                  <Button variant="secondary" size="sm" className="flex-shrink-0 text-xs">
                    {insight.action}
                  </Button>
                </Link>
              </div>
      ...
      {/* Quick actions */}
      <div className="glass-card p-6 animate-fade-up opacity-0" style={{ animationDelay: "500ms", animationFillMode: "forwards" }}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            { label: "Create Event", href: "/dashboard/events/create", icon: "✨", color: "from-brand-500/20 to-brand-700/20", border: "rgba(98,114,245,0.25)" },
            { label: "View All Events", href: "/dashboard/events", icon: "📅", color: "from-emerald-500/15 to-teal-500/15", border: "rgba(16,185,129,0.2)" },
            { label: "Edit Profile", href: "/dashboard/profile", icon: "👤", color: "from-violet-500/15 to-purple-500/15", border: "rgba(168,85,247,0.2)" },
            { label: "AI Suggestions", href: "/dashboard/events/create", icon: "🤖", color: "from-cyan-500/15 to-blue-500/15", border: "rgba(34,211,238,0.2)" },
          ].map(({ label, href, icon, color, border }) => (
            <Link key={label} href={href}>
              <div
                className={cn("rounded-xl p-4 text-center transition-all hover:scale-[1.03] cursor-pointer border bg-gradient-to-br", color)}
